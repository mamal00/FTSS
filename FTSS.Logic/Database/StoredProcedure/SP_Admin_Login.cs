...
namespace FTSS.Logic.Database.StoredProcedure
{
	public class SP_Admin_Login
	{
        ...
        public static async Task<Models.Database.DBResult> Call(IDBCTX ctx, AutoMapper.IMapper mapper,
            Models.Database.StoredProcedures.SP_Admin_Login_Params filterParams, string key, string issuer)
        {
            var connectionString = ctx.GetConnectionString();
            var sp = new FTSS.DP.DapperORM.StoredProcedure.SP_Admin_Login(connectionString);
            ...
            if(rst.ErrorCode==200)
			{
                ...
                var spAccessMenuResult = await spAccessMenu.CallAdmin(rst.Data as Models.Database.StoredProcedures.SP_Admin_Login);
