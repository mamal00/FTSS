...
{
	public class SP_Admin_Login : ISP<Models.Database.StoredProcedures.SP_Admin_Login_Params>
	{
        private readonly ISQLExecuter _ISQLExecuter;
        public SP_Admin_Login(string cns, ISQLExecuter ISQLExecuter = null)
        {
        ...
        public async Task<DBResult> Call(Models.Database.StoredProcedures.SP_Admin_Login_Params filterParams)
        {
            if (filterParams == null)
                throw new ArgumentNullException("خطا در نحوه ارسال درخواست");
            string sql = "dbo.SP_Admin_Login";
            DBResult rst = null;
            var p = Common.GetSearchParams();
            p.AddDynamicParams(Common.GenerateParams(filterParams, null));
            var dbResult = await _ISQLExecuter.QueryFirstOrDefaultAsync<Models.Database.StoredProcedures.SP_Admin_Login>(
